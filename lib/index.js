...
{"type":"task","task":"查一下今天的天气然后提醒我"}`
  ...
  const executeTask = async (taskText) => {
    ...
    // 优先尝试真正的 subagent 执行（方案三）
    try {
      const subagents = eventCtx.subagents
      const agents = eventCtx.agents
      if (subagents?.start && agents) {
        const parent = agents.roots?.()[0] || agents.list?.()[0]
        if (parent) {
          const provider = (subagents.list?.() || []).find((name) => name === 'spawn' || name === 'fork') || subagents.list?.()[0]
          if (provider) {
            const run = await subagents.start(provider, {
              label: 'better-dsh-pet-task',
              prompt: [{ type: 'text', text: `请实际执行以下任务（可以使用文件系统/命令/工具），完成后返回结果摘要：\n${input}` }],
              parent,
              signal: AbortSignal.timeout(120000),
            })
            const result = await run.result
            await run.dispose().catch(() => {})
            const output = (result.output || []).map((block) => block.text || '').join(' ').trim()
            if (output) return output
          }
        }
      }
    } catch {
      // subagent 不可用时走下面的独立 LLM fallback
    }
