...
export const inject = ['sessions', 'subagents', 'agents']
...
export const FESTIVAL_PLAY_ENDPOINT = '/plugins/better-dsh-pet/festival'
export const INTENT_ENDPOINT = '/plugins/better-dsh-pet/intent'
export const TASK_ENDPOINT = '/plugins/better-dsh-pet/task'
...
export const Config = Schema.object({
  ...
  holidayEnabled: Schema.boolean().default(false).description('启用节日祝福识别（阳历与农历）'),
  wakeWord: Schema.string().default('大肥鱼').description('语音唤醒词（例如：大肥鱼）'),
  taskCwd: Schema.string().default('').description('复杂任务工作目录（留空=用户主目录）'),
  weatherCity: Schema.string().default('').description('查天气默认城市（留空=自动定位）'),
...
function publicConfig(config = {}) {
  return {
    ...
    holidayEnabled: config.holidayEnabled ?? defaults.holidayEnabled,
    wakeWord: config.wakeWord || defaults.wakeWord,
    taskCwd: config.taskCwd || defaults.taskCwd,
    weatherCity: config.weatherCity || defaults.weatherCity,
  }
}
...
function mount(ctx, config = {}, eventCtx = ctx) {
  ...
  let roast = null
  let roastTimer = null
  let festivalPlayRequest = null
  let userClosed = false
  let lastEnabled = base.enabled !== false
  const taskHandles = new Map()
  let taskArchiveTimer = null
  let helperWatchdogTimer = null

  const getFestivalPlayRequest = () => {
    if (!festivalPlayRequest) return null
    if (festivalPlayRequest.expiresAt <= Date.now()) {
      festivalPlayRequest = null
      return null
    }
    return festivalPlayRequest
  }

  const queueFestivalPlayRequest = (festivalId) => {
    festivalPlayRequest = {
      token: randomUUID(),
      festivalId,
  ...
  const classifyIntent = async (text) => {
    ...
    const system = `你是命令分类器。判断用户这句话是“闲聊”还是“任务命令”。只输出 JSON，不要解释。
可用 action：pomodoro_start, pomodoro_stop, feed, hide, close, balance, roast, settings, open_web, remind, move_pet。
move_pet 的 args 用 {"position":"left|right|center|top|bottom"}。
输出示例：
{"type":"chat"}
{"type":"command","action":"pomodoro_start","args":{}}
{"type":"command","action":"open_web","args":{"url":"https://..."}}
{"type":"command","action":"move_pet","args":{"position":"left"}}
{"type":"command","action":"remind","args":{"text":"喝水","minutes":10}}
{"type":"task","task":"帮我在项目里搜索 xxx 并总结"}
{"type":"task","task":"查一下今天的天气然后提醒我"}`
    try {
      const response = await fetch('https://api.deepseek.com/chat/completions', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) return { type: 'chat' }
      const data = await response.json()
      const raw = data?.choices?.[0]?.message?.content || ''
      const match = raw.match(/\{[\s\S]*\}/)
      if (!match) return { type: 'chat' }
      const parsed = JSON.parse(match[0])
      if (parsed?.type === 'command') return { type: 'command', action: parsed.action, args: parsed.args || {} }
      if (parsed?.type === 'task') return { type: 'task', task: parsed.task || input }
      return { type: 'chat' }
    } catch {
      return { type: 'chat' }
    }
  ...
  const executeTask = async (taskText) => {
    const apiKey = await resolveApiKey()
    if (!apiKey) return '我还没连上 DeepSeek API，暂时无法执行任务~'
    const input = String(taskText || '').trim().slice(0, 500)
    if (!input) return '任务内容为空~'
    // 优先在 DSH 内打开一个新的真实对话，把任务作为首条消息，cwd 为当前工作目录
    try {
      const agents = eventCtx.agents
      if (agents?.create) {
        const parent = agents.roots?.()[0] || agents.list?.()[0]
        const sessionId = crypto.randomUUID()
        const cwd = settings.get().taskCwd?.trim() || homedir()
        const handle = await agents.create({
          sessionId,
          meta: { cwd },
          agentOptions: parent?.options,
        })
        taskHandles.set(sessionId, handle)
        handle.agent.followup({
          role: 'user',
          content: [{ type: 'text', text: `请实际执行以下任务（可以使用文件系统/命令/工具）：\n${input}` }],
          source: { kind: 'user' },
        })
        return `已在 DSH 中打开新对话并开始执行任务，会话 ID：${sessionId}`
      }
    } catch {
      // 创建新会话失败时走下面的独立 LLM fallback
    }
    ...
    try {
      const response = await fetch('https://api.deepseek.com/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'deepseek-chat',
          messages: [
            { role: 'system', content: '你是 better-dsh-pet 桌宠的任务执行助手。用户会给你一个任务请求。请在独立上下文中完成推理，并返回简短的结果摘要（不超过80字），不要询问额外问题，直接给出结果或明确的下一步建议。' },
            { role: 'user', content: input },
          ],
          max_tokens: 300,
          temperature: 0.7,
      ...
      if (!response.ok) return `任务执行接口出错了（${response.status}）`
      const data = await response.json()
      const reply = data?.choices?.[0]?.message?.content
      if (typeof reply === 'string' && reply.trim()) {
  ...
  const getStatus = () => ({
    ...
    festivalPlay: getFestivalPlayRequest(),
    tokenUsage,
    balance,
    roast,
  ...
  const startRuntime = (resolved) => {
    ...
    bridge = new HelperProcess({
      ...
      env: {
        ...
        DSH_PET_HOLIDAY_ENABLED: resolved.holidayEnabled === true ? '1' : '0',
        DSH_PET_WAKE_WORD: resolved.wakeWord || defaults.wakeWord,
        DSH_PET_TASK_CWD: resolved.taskCwd || '',
      },
    }, logger)
  ...
  const ensureHelperRunning = () => {
    try {
      if (settings.get().enabled === false) return
      if (userClosed) return
      if (!bridge) startRuntime(settings.get())
    } catch {
      // 启动失败时静默，等下一个周期再试。
    }
  }

  const stopTaskSession = async (sessionId, reason = 'session archived') => {
    const handle = taskHandles.get(sessionId)
    if (!handle) return
    taskHandles.delete(sessionId)
    try {
      // 先优雅取消：停止当前回合，保留会话内容，用户取消归档后还能继续看/用。
      handle.agent?.cancel?.({ kind: 'hook', reason: `better-dsh-pet: ${reason}` })
      // 如果取消后仍然在跑（例如正在执行外部命令/子代理），3 秒后强制销毁，杜绝“看似关了还在跑”。
      ...
      // 取消失败时直接销毁，避免后台继续烧 token。
      try { await handle.dispose?.() } catch { /* ignore */ }
    }
  }

  const checkTaskArchived = () => {
    const archived = new Set(eventCtx.workspaceRegistry?.archivedSessionIds || [])
    for (const [sessionId, handle] of taskHandles) {
      if (archived.has(sessionId)) {
        void stopTaskSession(sessionId, 'session archived')
      } else if (!eventCtx.agents?.get?.(sessionId)) {
        // 会话已不在 live registry（被删除/销毁），直接清理引用。
        taskHandles.delete(sessionId)
  ...
  const offDisposed = eventCtx.on('session/disposed', (session) => {
    if (taskHandles.has(session.id)) {
      const handle = taskHandles.get(session.id)
      taskHandles.delete(session.id)
      try { void handle?.dispose?.().catch?.(() => {}) } catch { /* ignore */ }
    }
    if (!bridge || !reducer) return
  ...
  if (!taskArchiveTimer) {
    taskArchiveTimer = setInterval(() => {
      try { checkTaskArchived() } catch { /* ignore */ }
    }, 2000)
    taskArchiveTimer.unref?.()
  }
  if (!helperWatchdogTimer) {
    helperWatchdogTimer = setInterval(() => ensureHelperRunning(), 5000)
    helperWatchdogTimer.unref?.()
  }
        ...
        () => httpCtx.webServer.register({
          ...
          handler: async (req, res) => {
            ...
            try {
              ...
              if (!festivalId) {
                jsonResponse(res, 400, { error: 'festivalId is required' })
                return
              }
              if (settings.get().holidayEnabled !== true) {
                jsonResponse(res, 409, { error: 'holidayEnabled is disabled' })
                return
              }
              const festival = queueFestivalPlayRequest(festivalId)
              jsonResponse(res, 200, { ok: true, festival })
            } catch (error) {
              jsonResponse(res, 400, { error: error instanceof Error ? error.message : String(error) })
            }
          },
        }),
        'better-dsh-pet: local festival play endpoint',
      )
      httpCtx.effect(
        () => httpCtx.webServer.register({
          kind: 'exact',
        ...
        () => httpCtx.webServer.register({
          ...
          handler: async (req, res) => {
            try {
              ...
              jsonResponse(res, 400, { ok: false, type: 'chat', error: error instanceof Error ? error.message : String(error) })
            }
          },
        }),
        'better-dsh-pet: local intent endpoint',
      )
      httpCtx.effect(
        () => httpCtx.webServer.register({
          kind: 'exact',
          ...
          handler: async (req, res) => {
            try {
              ...
              jsonResponse(res, 400, { ok: false, result: error instanceof Error ? error.message : String(error) })
            }
          },
        }),
        'better-dsh-pet: local task endpoint',
      )
      httpCtx.effect(
        () => httpCtx.webServer.register({
          kind: 'exact',
  ...
  ctx.effect(() => () => {
    ...
    if (roastTimer) clearTimeout(roastTimer)
    roastTimer = undefined
    if (taskArchiveTimer) clearInterval(taskArchiveTimer)
    taskArchiveTimer = undefined
    if (helperWatchdogTimer) clearInterval(helperWatchdogTimer)
    helperWatchdogTimer = undefined
