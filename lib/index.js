...
export const Config = Schema.object({
  ...
  festivalSimDate: Schema.string().default('').description('节日模拟日期（YYYY-MM-DD，留空=今天，用于不改系统时间测试节日）'),
...
function publicConfig(config = {}) {
  return {
    ...
    festivalSimDate: config.festivalSimDate ?? defaults.festivalSimDate,
  }
}
  ...
  const startRuntime = (resolved) => {
    ...
    bridge = new HelperProcess({
      ...
      env: {
        ...
        DSH_PET_FESTIVAL_SIM_DATE: resolved.festivalSimDate || '',
      },
    }, logger)
