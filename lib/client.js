  ...
  factory: (require) => {
    ...
    function PetSettingsCard() {
      ...
      return h('li', { style: cardStyle, 'data-testid': 'better-dsh-pet-settings' },
        h('div', null,
          h('strong', { style: { fontSize: 16 } }, '大肥鱼桌面伴侣'),
          h('p', { style: { margin: '5px 0 0', opacity: 0.72 } }, '独立桌面气泡，由 DSH 真实工作状态驱动；网页内不再显示浮动大肥鱼。'),
        ),
        status === 'unavailable'
          ? h('span', { role: 'status' }, '大肥鱼设置尚未连接到 DSH Host。')
          : status === 'loading'
          ? h('span', null, '正在读取设置…')
          : h(React.Fragment, null,
            h(Field, { label: '启用大肥鱼', hint: '关闭后立即退出桌面气泡；重新开启无需单独启动程序。' },
              h('input', {
            ...
            h(Field, { label: '宠物宽度（px）', hint: '基础尺寸，配合“角色大小”百分比使用，重启后生效。' },
              h('input', {
                ...
                value: value.petSize ?? 462, disabled: !writable, style: selectStyle,
